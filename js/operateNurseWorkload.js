...
function insertSSHSGZLTable(){
	...
	for(var i=0;i<SSHSGZLdataSource.length;i++){
		...
		var tdIndexTemp = (SSHSGZLpage-1) * SSHSGZLnumPer + i + 1;
		if(SSHSGZLTopList.indexOf(tdIndexTemp) != -1){
		...
		$(span).click(param, function(event){
			...
			if(SSHSGZLTopList.indexOf(tdIndex) == -1){
				$('#SSHSGZL_table_top').prepend($(this).parent().parent().clone(true));
				$(this).css('background-color', 'yellow');
				//$(this).css('visibility', 'hidden');

				alert('成功置顶');
				SSHSGZLTopList.push(tdIndex)
			}else{
				alert('该项已置顶');
			}
		});
